...
class ServerDeployer:
    ...
    def create_fl_server(self, args, secure_train=False):
        ...
        services = FederatedServer(
            ...
            disabled_client_fail_open=first_server.get("disabled_client_fail_open"),
        )
    ...
    def _initialize_state_store(self, services: FederatedServer, server_root: str):
        ...
        ensure_sqlite_parent_dir(store.db_url)
        result = prepare_server_state_store(store, server_root, self._resolve_legacy_jobs_dir(server_root))
        ...
        elif result["action"] == "imported_registry":
            self.logger.info(
                f"state store bootstrapped from provisioned study registry '{result['registry_path']}': "
                f"imported studies {result['imported_studies']}"
            )
    ...
    def _resolve_legacy_jobs_dir(self, server_root: str) -> Optional[str]:
        ...
        if not isinstance(job_store, FilesystemStorage):
            ...
            return None
        try:
            return job_store.object_path(uri_root)
