...
class ClientManager:
    ...
    def _is_client_disabled(self, client_name: str) -> bool:
        return self._require_state_store().get_disabled_client(client_name) is not None

    def is_client_disabled(self, client_name: str) -> bool:
        return self._is_client_disabled(client_name)
    ...
    def enable_client(self, client_name: str) -> bool:
        with self.lock:
            was_disabled = self._is_client_disabled(client_name)
            if was_disabled:
                self._require_state_store().enable_client(client_name)
    ...
    def authenticated_client(self, request, fl_ctx: FLContext, client_type) -> Optional[Client]:
        ...
        if self._is_client_disabled(client_name):
