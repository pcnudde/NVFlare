...
class TestTensorDownloadableBasic:
    ...
    def test_lazy_refs_are_served_from_the_file_without_prefetch_or_cache(self, tmp_path):
        tensors = {"a": torch.arange(4.0), "b": torch.ones(2)}
        save_file(tensors, tmp_path / "model.safetensors")
        downloadable = TensorDownloadable(
            tensors=safetensors_refs(str(tmp_path / "model.safetensors")), max_chunk_size=1
        )
        ...
        rc, items, _ = downloadable.produce(state, "receiver")
        assert torch.equal(load_tensors(bytes(items[0]))["b"], tensors["b"])
        downloadable.release()
